...
from vector_interpolation import double_vector_interp


'''Function that is used for sorting the Active_boundary_points array in the function g_shading.
   This function will be used only if the length of Active_boundary_points is 2 and sorts objects 
   of the class BoundaryPoints with respect to the x value of theiw point attribute.'''
def BoundaryPointSortx(ABPL):
    if len(ABPL) ==2:
        if ABPL[0].point[0] > ABPL[1].point[0]:  #If x value of point1 is greater than point2
            ABPL[0] , ABPL[1] = ABPL[1] , ABPL[0]  #swap




'''g_shading is the function responsible for the gouraud shading of a triangle given the coordinates of its vertices,
   their color and an image on which the triangle will be projected. This function is identicall to the f_shading 
   function except at the part where the pixel color is drawn.'''
    ...
    for y in range (ymin,ymax+1):  #for every y where the triangle lies
        ...
        if len(Active_boundary_points)==2:
            ...
            for x in range (x_value_of_boundary_points[0] , x_value_of_boundary_points[1]):
                #draw pixel using double_vector_interp function
                BoundaryPointSortx(Active_boundary_points)  #sort the boundary points with respect to x so that the first element is the leftmost
                img[y][x]= double_vector_interp([x,y], Active_boundary_points[0], Active_boundary_points[1])
        else:
            return
